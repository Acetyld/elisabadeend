<template>
    <div class="flex flex-col items-center justify-center min-h-screen bg-yellow-200 p-4">
      <div class="bg-white shadow-md rounded-lg p-6 w-full max-w-2xl">
        ...
        <ul class="space-y-2">
          ...
          <li v-for="game in games" :key="game.id" class="flex items-center text-yellow-600 hover:text-yellow-800">
  ...
  export default {
    ...
    data() {
      return {
        games: [],  // Hier gaan we de opgehaalde games opslaan
      };
    },
    mounted() {
      this.fetchGames();  // Haal de games op zodra de component geladen is
    },
    methods: {
      async fetchGames() {
        try {
          ...
          this.games = response.data.data;  // Sla de opgehaalde games op
        } catch (error) {
